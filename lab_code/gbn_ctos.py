...
class GBN:
    ...
    def __init__(self, local_address=Host.host_address_1, remote_address=Host.host_address_2):
        ...
        self.data = []  # 缓存发送数据
        self.read_path = '../clientfile/client_upload.txt'  # 需要发送的源文件数据
        self.ack_buf_size = 10
        self.get_data_from_file()

        self.data_buf_size = 1678  # 作为服务器接收数据缓存
        self.exp_seq = 0  # 当前期望收到该序号的数据
        self.save_path = '../serverfile/server_upload.txt'  # 接收数据时，保存数据的地址
    ...
    # 主要执行函数，不断接收客户端发送的数据，若为期待序号的数据，则保存到本地，否则直接丢弃；并返回相应的ACK报文
    def server_run(self):
        while True:
            readable = select.select([self.socket], [], [], 1)[0]  # 监听可读事件
            if len(readable) > 0:  # 接收到数据
                rcv_data = self.socket.recvfrom(self.data_buf_size)[
                    0].decode()  # 使用socket.recvfrom方法接收数据，并使用decode方法将二进制数据解码成字符串
                ...
                if int(rcv_seq) == self.exp_seq:  # 如果接收到的数据序号与期望的序号一致，则表示接收到按序的数据包，打印信息并将数据保存到本地文件中，并更新期望数据的序号
                    print('服务器:收到期望序号数据:' + str(rcv_seq))
                    self.write_data_to_file(rcv_data)  # 保存客户端发送的数据到本地文件中
                    self.exp_seq = self.exp_seq + 1  # 期望数据的序号更新
                else:
                    print('服务器:收到非期望数据，期望:' + str(self.exp_seq) + '实际:' + str(
                        rcv_seq))  # 如果接收到的数据序号与期望的序号不一致，则表示接收到了非期望的数据包，打印信息显示期望序号和实际序号
