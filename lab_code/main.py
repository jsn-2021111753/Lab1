...
def run_gbn():
    ...
    choice = input('请选择上传upload，还是下载download')
    if choice == 'download':
        from lab_code.gbn_stoc import GBN
    ...
    if choice == 'upload':
        from lab_code.gbn_ctos import GBN
        host_1 = GBN(Host.host_address_1, Host.host_address_2)
        host_2 = GBN(Host.host_address_2, Host.host_address_1)
        # upload
        threading.Thread(target=host_1.server_run).start()  # 注意这里函数一定不能带括号
...
def run_sr():
    ...
    choice = input('请选择上传upload，还是下载download')
    if choice == 'download':
        from lab_code.sr_stoc import SR
    ...
    if choice == 'upload':
        from lab_code.sr_ctos import SR
        host_1 = SR(Host.host_address_1, Host.host_address_2)
        host_2 = SR(Host.host_address_2, Host.host_address_1)
        # upload
        threading.Thread(target=host_1.server_run).start()  # 注意这里函数一定不能带括号
